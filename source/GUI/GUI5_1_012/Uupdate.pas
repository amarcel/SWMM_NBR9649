...
{                                                                   }
{   5.1.010:                                                        }
{   - When converting link offsets from depth to elevation, if      }
{     the offset is 0 then the node invert elevation is used        }
...
begin
  Uutils.GetSingle(L.Data[I], Z);
  if Z = 0 then Result := N.Data[NODE_INVERT_INDEX]   //'*'                    //(5.1.010)
  else
  begin
    Uutils.GetSingle(N.Data[NODE_INVERT_INDEX], E);
    Z := Z + E;
...
begin
  ...
  begin
    ...
    begin
      if Uutils.MsgDlg(Msg1, mtConfirmation, [mbYes, mbNo]) = mrYes            //(5.1.008)
  ...
  begin
    if Project.GetLinkCount > 0 then
    begin
      if Uutils.MsgDlg(Msg2, mtConfirmation, [mbYes, mbNo]) = mrYes            //(5.1.008)

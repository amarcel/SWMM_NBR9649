...
begin
  ...
  begin
    ...
    if not RenameFile(TmpFname, Fname[3]) then
      Uutils.MsgDlg('Could not save combined file ' + Fname[3] + '.',          //(5.1.008)
        mtError, [mbOK], MainForm)                                             //(5.1.008)
    else Uutils.MsgDlg('A total of ' + IntToStr(Ndates) +                      //(5.1.008)
      ' records were combined into file ' + ExtractFileName(Fname[3]) + '.',
      mtInformation, [mbOK], MainForm);                                        //(5.1.008)
    Result := True;
  end;
end;

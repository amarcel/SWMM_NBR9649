...
{   formatted text file.                                            }
{                                                                   }
{   Release 5.1.008 - column headings were changed to better match  }
{                     those used in Appendix D of the Users Manual. }
{                                                                   }
{   Release 5.1.010 - changes made to accommodate the new Modified  }
{                     Green-Ampt infiltration and Roadway weir      }
{                     options.                                      }
{                                                                   }
{   Release 5.1.011 - dates of detailed hydraulic events are        }
{                     written to an [EVENTS] section of the file.   }
{                                                                   }
{-------------------------------------------------------------------}
...
begin
  ...
  begin
    ...
    begin
      if Length(TmpOptions[I]) = 0 then continue;                              //(5.1.008)
      Line := Format('%-20s', [OptionLabels[I]]) + Tab + TmpOptions[I];
      S.Add(Line);
    end;
  end;
...
////  New procedure added for release 5.1.007.  ////                           //(5.1.007)
...
var
  ...
  CondAdjusted: Boolean;                                                       //(5.1.008)
begin
  TempAdjusted := False;
  EvapAdjusted := False;
  RainAdjusted := False;
  CondAdjusted := False;                                                       //(5.1.008)
  with Project.Climatology do
  begin
    for I := 0 to 11 do
    begin
      ...
      if Length(CondAdjust[I]) > 0 then CondAdjusted := True;                  //(5.1.008)
    end;
  end;
  if (TempAdjusted) or (EvapAdjusted) or (RainAdjusted) or (CondAdjusted) then //(5.1.008)
  begin
    S.Add('');
    S.Add('[ADJUSTMENTS]');
...
////  New procedure added for release 5.1.011.  ////                           //(5.1.011)
...
begin
  ...
  K := -1;
  if SameText(InfilType, InfilOptions[HORTON_INFIL])                           //(5.1.010)
  or SameText(InfilType, InfilOptions[MOD_HORTON_INFIL]) then                  //(5.1.010)
  begin
  ...
  else if SameText(InfilType, InfilOptions[GREEN_AMPT_INFIL])                  //(5.1.010)
       or SameText(InfilType, InfilOptions[MOD_GREEN_AMPT_INFIL]) then         //(5.1.010)
  begin
    Line := ';;Subcatchment  ' + Tab + 'Suction   ' + Tab + 'Ksat      ' + Tab + 'IMD       ';
    S.Add(Line);
    Line := ';;--------------' + Tab + '----------' + Tab + '----------' + Tab + '----------';
    S.Add(Line);
    K := 2;
  end
  else if SameText(InfilType, InfilOptions[CURVE_NUMBER_INFIL]) then           //(5.1.010)
  begin
    Line := ';;Subcatchment  ' + Tab + 'CurveNum  ' + Tab + '          ' + Tab + 'DryTime   ';
    S.Add(Line);
    Line := ';;--------------' + Tab + '----------' + Tab + '----------' + Tab + '----------';
    S.Add(Line);
    K := 2;
...
////  Modified to preserve backwards compatibility with SWMM 5.0  ////         //(5.1.007)
...
////  The following code section was modified for release 5.1.007.             //(5.1.007)
...
begin
  ...
  S.Add('');
  S.Add('[JUNCTIONS]');
  Line := ';;Name          ' + Tab + 'Elevation ' + Tab + 'MaxDepth  ';        //(5.1.007)
  Line := Line + Tab + 'InitDepth ' + Tab + 'SurDepth  ' + Tab + 'Aponded   '; //(5.1.007)
  S.Add(Line);
  Line := ';;--------------' + Tab + '----------' + Tab + '----------';
  Line := Line + Tab + '----------' + Tab + '----------' + Tab + '----------';
  S.Add(Line);
...
begin
  ...
  with Project.Lists[OUTFALL] do
    ...
    begin
      ...
      Line := Line + Tab + Format('%-8s', [N.Data[OUTFALL_TIDE_GATE_INDEX]]);  //(5.1.008)
      Line := Line + Tab + Format('%-16s', [N.Data[OUTFALL_ROUTETO_INDEX]]);   //(5.1.008)
      S.Add(Line);
    end;
end;
...
////  This procedure was re-written for release 5.1.007.  ////                 //(5.1.007)
...
begin
  ...
  with Project.Lists[WEIR] do
    ...
    begin
      ...
      Line := Line + Tab + Format('%-10s', [L.Data[WEIR_SURCHARGE_INDEX]]);    //(5.1.007)

      if SameText(L.Data[WEIR_TYPE_INDEX], 'ROADWAY') then                     //(5.1.010)
      begin
...
begin
  ...
  Ulid.ExportLIDGroups(S, Tab);                                                //(5.1.008)
  ExportAquifers(S);
  ExportGroundwater(S);
  ExportSnowpacks(S);
  ...
  ExportPatterns(S);
  ExportReport(S);
  ExportAdjustments(S);                                                        //(5.1.007)
  ExportEvents(S);                                                             //(5.1.011)
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          // Write node lateral inflow to file                                 //(5.1.008)
          V := GetVarIndex(LATFLOW, NODES);                                    //(5.1.008)
          X := Uoutput.GetNodeOutVal(V, CurrentPeriod, K)/QCF;                 //(5.1.008)
          BlockWrite(F, X, SizeOf(X));
...
/////////////////////////////////////////
    ...
    on EInOutError do
      Uutils.MsgDlg('I/O error writing to hotstart file.', mtError, [mbOK]);   //(5.1.008)
  end;
  CloseFile(F);
end;

...
begin
  ...
  begin
    Uutils.MsgDlg(MSG_NO_IMAGE_FILE + Fname, mtWarning, [mbOK]);               //(5.1.008)
    Exit;
  end;
  ...
  if Result = False
  then Uutils.MsgDlg(MSG_BAD_IMAGE_FILE + Fname, mtWarning, [mbOK]);           //(5.1.008)
end;
...
begin
  ...
  if not FileExists(Fname) then
  begin
    Uutils.MsgDlg(MSG_NO_WORLD_FILE + Fname, mtWarning, [mbOK]);               //(5.1.008)
    Exit;
  end;
  ...
  begin
    Uutils.MsgDlg(MSG_BAD_WORLD_FILE + Fname, mtWarning, [mbOK]);              //(5.1.008)
    Exit;
  end;
    ...
    if ScaleMapCheckBox.Checked then Scaling := SCALE_TO_BACKDROP
      ...
      begin
        ...
        // See if user wants to scale backdrop to map
          case Uutils.MsgDlg(MSG_NO_OVERLAP, mtConfirmation,                   //(5.1.008)

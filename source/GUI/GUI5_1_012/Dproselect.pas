...
{   a profile plots.                                                }
{                                                                   }
{   5.1.009: Changed default position of form.                      }
{-------------------------------------------------------------------}
...
uses
  ...
  Vcl.StdCtrls, Vcl.Buttons, Contnrs, Uglobals, Uproject, Uutils;              //(5.1.008)
...
begin
  Left := MainForm.Left + (MainForm.Width - MainForm.ClientWidth) - 2;         //(5.1.009)
  Top := MainForm.Top + (MainForm.Height-MainForm.ClientHeight) - 2;           //(5.1.009)
...
begin
  ...
  if I >= 0 then
  begin
    Uutils.MsgDlg('Link ' + LinksListBox.Items[I] +                            //(5.1.008)
...
begin
  ...
  begin
    Uutils.MsgDlg(MSG_START_NODE_NOT_EXIST, mtError, [mbOK]);                  //(5.1.008)
    exit;
  end;
  StartNode := Project.GetNode(I, J);
  if not Project.FindNode(EndNodeEdit.Text, I, J) then
  begin
    Uutils.MsgDlg(MSG_END_NODE_NOT_EXIST, mtError, [mbOK]);                    //(5.1.008)
    exit;
  end;
  ...
  try
    ...
    then Uutils.MsgDlg(MSG_NO_VALID_PATH, mtError, [mbOK])                     //(5.1.008)

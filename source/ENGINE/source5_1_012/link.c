...
static double nbr9649_getCriticalVeloc(double hydRadius);
static double nbr9649_getFinalVeloc(double q, double wetArea);
static double nbr9649_getTrativeTension(double hydRadius, double conduitSlope);
static double nbr9649_getPercLevel(double y, double yFull);
static double nbr9649_getMinSlopeI0Min(double q);
static double nbr9649_getMaxSlopeI0Max(double q);
...
//
{
	...
	hydRadius = xsect_getRofY(&Link[j].xsect, y);
	conduitSlope = conduit_getSlope( j );
	...
	x[LINK_HYD_RADIUS]   = (float) hydRadius;
	x[LINK_MIN_SLOPE]    = (float) nbr9649_getMinSlopeI0Min( q );
	x[LINK_MAX_SLOPE]    = (float) nbr9649_getMaxSlopeI0Max( q );
	x[LINK_CRIT_VELOC]   = (float) nbr9649_getCriticalVeloc( hydRadius );
	x[LINK_TRAT_TENSION] = (float) nbr9649_getTrativeTension( hydRadius, conduitSlope );
	x[LINK_PERC_LEVEL]   = (float) nbr9649_getPercLevel( y, Link[j].xsect.yFull );
...
//=============================================================================
//                    N B R 9 6 4 9   M E T H O D S
//=============================================================================

double nbr9649_getCriticalVeloc(double hydRadius)
//
//  Input:   hydRadius = hydraulic radius
//  Output:  returns critical velocity (NBR9649)
//  Purpose: computes critical velocity for given hydraulic radius.
//
{
	return (double) 6.0 * sqrt(9.81 * hydRadius);
}

//=============================================================================

double nbr9649_getTrativeTension(double hydRadius, double conduitSlope)
//
//  Input:   hydRadius    = hydraulic radius
//           conduitSlope = conduit slope by link j
//  Output:  returns trative tension (NBR9649)
//  Purpose: computes trative tension for given hydraulic radius and conduit slope.
//
{
	return (double) 10000.0 * hydRadius * conduitSlope;
}

//=============================================================================

double nbr9649_getPercLevel(double y, double yFull)
//
//  Input:   y = depth
//           yFull = depth when full (ft)
//  Output:  returns percentage level (NBR9649)
//  Purpose: computes percentage level for given depth and conduit link length.
//
{
	return (double) y / yFull;
}

//=============================================================================

double nbr9649_getMinSlopeI0Min(double q)
//
//  Input:   q = flow
//  Output:  returns min. slope (NBR9649)
//  Purpose: computes min. slope for given flow.
//
{
	return (double) q >= 1.5 ? 0.0055 * pow(q, -0.47) : 0.0055 * pow(1.5, -0.47);
}

//=============================================================================

double nbr9649_getMaxSlopeI0Max(double q)
//
//  Input:   q = flow
//  Output:  returns max. slope (NBR9649)
//  Purpose: computes max. slope for given flow.
//
{
	return (double) q >= 1.5 ? 4.5 * pow(q, -0.67) : 4.5 * pow(1.5, -0.67);
}
